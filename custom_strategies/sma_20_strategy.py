...
class smaStrategy:
    ...
    def timeInd(self, symbolID=0):
        ts = self.td.time_series(symbol=self.symbols[symbolID], interval='1day', outputsize=5)
        ti = ts.with_sma(time_period=20).with_rsi(time_period=10).as_pandas()
        ti['symbol'] = self.symbols[symbolID]
        #trying to make up the trade signal buy/sell/None
        if not ti.empty and ti['low'][-1] > ti['sma20'][-1] \
                and ti['open'][0] > ti['sma20'][-1]
            ti['signal'] = 'buy'
        else:
            ti['signal'] = 'sell'
